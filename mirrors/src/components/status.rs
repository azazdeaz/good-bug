...
impl Status {
    pub fn new(owner: TRef<Node>, path: String, context: &mut Context) -> Self {
        ...
        {
            let mut recv_toggled = context.signal_map.connect_b(owner, &enable_auto_switch_path, "toggled");
...
impl Updatable for Status {
    fn update(&self, owner: &Node) {
        ...
        if let Some(enable_auto_nav) = enable_auto_nav {
            let enable_auto_switch = get_node::<CheckButton>(owner, self.enable_auto_switch_path.clone());
