...
impl Game {
    ...
    #[export]
    fn set_target(&mut self, owner: TRef<Node>, x: f64, y: f64, z: f64) {
    ...
    #[export]
    fn set_follow_target(&mut self, owner: TRef<Node>, on: bool) {
    ...
    #[export]
    fn set_step(&mut self, owner: TRef<Node>, left: f64, right: f64, time: f64) {
        self.values.step = Some((left, right, time));
    }

    #[export]
    fn set_scale_factor(&mut self, owner: TRef<Node>, scale: f64) {
        self.values.scale_factor = scale;
    }

    #[export]
    fn set_min_lm_obs(&mut self, owner: TRef<Node>, min_lm_obs: f64) {
        self.values.min_lm_obs = min_lm_obs as u32;
    }

    #[export]
    fn toggle_connection(&mut self, owner: TRef<Node>, on: bool) {
        let msg = if on {
            "start_publisher"
        } else {
            "pause_publisher"
    ...
    #[export]
    fn request_slam_terminate(&mut self, owner: TRef<Node>) {
    ...
    unsafe fn _process(&mut self, owner: &Node, _delta: f64) {
        ...
        if let Some(rx) = &self.rx {
            while let Ok(msg) = rx.try_recv() {
                ...
                //                 }
                //             }
                //             owner.emit_signal("message", &[Variant::from_str(text)]);
                //         }
                ...
                //         //     owner.emit_signal("current_frame", &[current_frame.to_variant()]);
                //         // }
